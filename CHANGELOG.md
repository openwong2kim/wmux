...
- **Mission control: your agents, the orchestrator, and their vitals now live in one place.** The Orchestrator tab opens with a **Fleet roster** pinned above the thread — one row per live terminal pane showing a status dot (amber running, red needs-input, gray idle), the pane's name, and what it's doing right now (the same hook-driven activity line the cockpit cards use); click any row to jump straight to that pane. And a new **instrument strip** runs along the very bottom of the window — orchestrator model on the left, "N running · N need you" on the right — so an agent that needs you is visible from any workspace, any tab; clicking "need you" jumps to the most urgent pane.
