...
- **Pin an agent and model to each pane role, and wmux enforces it at launch.** A pane role (Builder / Reviewer / Tester / Planner) used to be a soft routing hint the orchestrator could read but nothing acted on. You can now bind each role to an agent + model in Settings › Orchestrator — e.g. Reviewer → `codex` / a cheap model — and any agent launched in a pane carrying that role is transparently rewritten to run with the bound model, whether the orchestrator issued it (`terminal_send("claude")` becomes `claude --model haiku`) or you typed the launcher yourself. Enforcement happens at wmux's own command-assembly points: the main-process input chokepoint that every submitted launch flows through, the seeded launch command of a restored project pane, and the per-pane resume command (so a bound model survives a reboot instead of silently dropping). The rewrite is loud and reversible — you see the real command in the pane and a muted `agent · model` badge in the Fleet roster and on the pane — and it never fights an explicit `--model` you put on the line (a manual override wins for that one launch; the binding re-asserts on the next respawn). Roles are unbound by default. Agents whose model-flag grammar isn't verified (only `claude` and `codex` are today) are left unchanged with an advisory note rather than given a guessed flag. Per-role budgets and token metering are not part of this change.
