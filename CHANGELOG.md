...
- **The Resume chip has a `--dangerously-skip-permissions` toggle (Claude only, on by default).** Expanding the chip now shows a checkbox that forces `--dangerously-skip-permissions` onto the resume command it types — the common case for anyone who runs Claude in bypass mode. It's checked by default and the command preview updates live as you toggle it; unchecking falls back to restoring the conversation's captured permission mode (e.g. `--permission-mode acceptEdits`) on an exact resume. The flag rides both the exact `--resume <id>` and the cwd-relative `--continue` fallback, since it's a launch preference rather than something tied to one conversation. Codex, which has no equivalent flag, doesn't show the toggle.
