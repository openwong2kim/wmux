...
- **`wmux web` — reach your terminal panes from a phone browser (read-only by default).** A new command starts a small web server, hosted inside the daemon, that serves your live panes to any browser on the same Tailscale tailnet (or LAN): run `wmux web`, open the printed `http://<tailnet-ip>:7681/?token=…` link on your phone, pick a pane, and watch its output stream in real time. It reuses the daemon's own PTY data (initial screen from the same ring-buffer flush the desktop uses, then a live tee of terminal output), so it never fights the desktop GUI for a pane and it keeps working even after you close the desktop app — the agents are still supervised by the daemon. It is **read-only by default**: the browser can watch but cannot type, preserving wmux's "remote can't execute" posture. Pass `--allow-input` to explicitly enable keyboard input when you want to steer from your phone. Access is gated by a fresh random token minted on each start (the daemon's own control token never touches the network), and nothing listens until you run the command. The page is installable: iOS "Add to Home Screen" works over plain HTTP, and full PWA install (Android) plus offline app-shell caching work when the server is fronted with HTTPS (e.g. `tailscale serve`). Options: `--port` (default 7681), `--host` (default `0.0.0.0`), `--status`, `--stop`.
