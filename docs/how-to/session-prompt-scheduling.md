...
- Prompts use sanitized bracketed paste, followed by a separate submit write. The daemon re-verifies identity, readiness, and the input revision during that delay; concurrent human input cancels Enter and records an error instead of submitting mixed text. Multiline prompts use the same protected delivery path as other structured wmux messages.
