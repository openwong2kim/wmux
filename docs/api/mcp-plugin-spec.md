...
- **Config changes *who* is recognised, never *what* they may call.** `FIRST_PARTY_METHODS` stays compiled. A configured name gets exactly the same curated method set, and the same three guards (explicit `denied` wins, failed trust read declines, method outside the set falls through).
...
- **wmux's own internal tiers.** `wmux-cli` has a deliberately *narrower* allowlist (`WMUX_CLI_METHODS`) and is checked *after* the first-party branch, so configuring it would silently swap the CLI's curated set for the larger first-party one. `unknown` is wmux's placeholder for envelope-less callers and appears verbatim in real trust DBs.
...
**Finding the observed name.** Identity rejections echo the `clientName` wmux actually saw, and `wmux mcp clients` lists every client in the trust DB with a `NOT CONFIGURABLE` marker on non-identifying names. Both exist because the rejection alone previously said only "plugin is unconfirmed": the operator had no supported way to learn which name to allowlist, which led a real agent to guess its own name wrong. The CLI reads `plugin-trust.json` directly so it works with the app closed — the state you are in when editing `config.json`.

Reference: `src/main/mcp/firstParty.ts` (allowlist, denylist gate, `isFirstPartyClient`), `src/main/mcp/firstPartyConfig.ts` (the config read), `src/main/mcp/PermissionEnforcer.ts` (the gated bypass branch), `src/cli/commands/mcp.ts` (`wmux mcp clients`), `plans/first-party-mcp-trust.md` (the design and why the token/declare alternatives were rejected).
