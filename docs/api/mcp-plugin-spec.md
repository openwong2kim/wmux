...
- **wmux's own internal tiers.** `wmux-cli` has a deliberately *narrower* allowlist (`WMUX_CLI_METHODS`) and is checked *after* the first-party branch, so configuring it would silently swap the CLI's curated set for the larger first-party one. `wmux-hook-bridge` (`HOOK_BRIDGE_METHODS`, exactly `hooks.signal`) is narrower still, for the same reason. `unknown` is wmux's placeholder for envelope-less callers and appears verbatim in real trust DBs.
