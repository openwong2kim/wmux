...
- Cursor is **opaque**. Clients pass `nextCursor` back verbatim on the next poll. Do not increment it or clamp it against the previous cursor.
...
- With `resync: true`, `nextCursor` is a replacement cursor and may be lower than `priorCursor`, including `0` against an empty ring. See [`PROTOCOL.md` §2.2](../PROTOCOL.md#22-cursor-is-opaque).
...
- `resync: true` indicates the caller's cursor drifted past the ring window or points past the newest event. Client must reconcile via `pane.list`.
