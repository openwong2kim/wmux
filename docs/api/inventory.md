...
| `pane.stash` | `{ id }` | stable (#977) | Takes a leaf pane out of the layout WITHOUT killing it — the daemon keeps the session and replays it on return. Refused when the pane is the only visible one, when there is no daemon connection, or when it holds an editor/diff tab whose unsaved state cannot be replayed. |
