...
|---|---|---|
| `browser_tabs` | 1 | Manages logical browser surfaces only in the calling session's workspace. `list`/`new` return stable opaque `surfaceId` values used by `select`/`close`; the former experimental numeric `tabId` index is no longer accepted. On the Live Chrome attach it also carries the agent-window write policy: rows report `owner` (`agent` / `borrowed` / `user`), `list` takes a `scope` filter, and `borrow` / `return` ask the user to lend the agent one of their own tabs (see [browser-backends.md](../browser-backends.md)). |
