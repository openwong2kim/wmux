...
For compatibility with daemons that implemented a subscribe-first backlog before they implemented the first-party gate, the app pipelines subscribe and identify as its first two authenticated RPCs, in that order. A gated daemon answers the first subscribe with the successful RPC envelope `{ id, ok: true, result: { ok: false } }`; after the separate identity RPC succeeds, the client immediately retries subscribe. The daemon retains a per-connection backlog bounded by 1 MiB and 15 seconds between accepting the socket and completing this handshake. Once subscribe has declared event-delivery intent, intervening ordinary RPCs do not discard retained events; only a socket whose first authenticated request is unrelated is committed to reply-only operation. A successful subscribe flushes the frames in order before its reply. If the byte bound overflows or the flush cannot complete, the daemon closes the socket so the app reconnects and rehydrates rather than continuing after silent partial loss; expiry degrades a late subscriber to live-only delivery without closing an otherwise usable RPC socket. A client that intends to receive pushed events MUST send the two handshake requests before ordinary RPCs, MUST inspect `result.ok`, and MUST correlate replies by `id`, including while a subscribe request itself is pending. An RPC-level failure such as an unknown method instead uses `{ id, ok: false, error }`.
...
Either default can be overridden explicitly via the config key. The enforcement decision is computed by a single pure function (`PermissionEnforcer.check`) shared by both modes, so shadow logs predict exactly what enforce mode would reject. Requests without a `clientName` envelope are grandfathered as `legacy` (§4.1) until the lane closes in the first release on or after **2026-09-30** (#1111); from that release they are refused.
...
Requests without `clientName` **on the external wire** are recorded as `legacy` — the trusted in-process surfaces (renderer `operator` bridge, iframe plugin host) send no `clientName` by design and are not recorded. The recorder stays on as the evidence trail, but **the grandfather lane closes in the first release on or after 2026-09-30** (#1111). From that release an envelope-less wire request is refused by the permission gate with `rejection.reason = 'identity-status'`, `status = 'legacy'`; there is no approval path back in, because an anonymous caller has no identity to approve. Two honestly-labelled limited lanes remain in its place, each a curated allowlist with no approval dialog: `wmux-cli` (the bundled CLI, `WMUX_CLI_METHODS`) and `wmux-hook-bridge` (the agent lifecycle bridges under `integrations/**`, granted exactly `hooks.signal` — see `src/main/mcp/hookBridge.ts`). Both names are in `NON_IDENTIFYING_CLIENT_NAMES`: anyone may send them, which is precisely why each grants a bounded method set rather than a blanket bypass. The in-process surfaces are unaffected by the close: they never carried an envelope and are exempt by positive in-process provenance, not by the grandfather.
