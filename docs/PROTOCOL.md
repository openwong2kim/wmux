...
- `nextCursor` returned from `events.poll` is the right value to pass back on the next poll. Pass it back verbatim; do not increment it or clamp it against an earlier cursor.
...
- **Replacement during resync.** On a response carrying `resync: true`, `nextCursor` is a replacement, not an advancement, and MAY be lower than `priorCursor` — including `0` against an empty ring. This re-anchors a cursor that pointed past the newest event (`cursor > latestSeq`, §2.5); a cursor that merely drifted past the ring window still advances. Pass the replacement back verbatim. A client that clamps it with `cursor = Math.max(cursor, nextCursor)` never re-anchors and repeats the same resync page indefinitely.
