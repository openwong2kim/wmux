...
Requests without `clientName` are recorded as `legacy` — the recorder stays on as the evidence trail — but **the grandfather lane closes in the first release on or after **2026-09-30** (#1111)**. From that release an envelope-less request is refused by the permission gate with `rejection.reason = 'identity-status'`, `status = 'legacy'`; there is no approval path back in, because an anonymous caller has no identity to approve. Two honestly-labelled limited lanes remain in its place, each a curated allowlist with no approval dialog: `wmux-cli` (the bundled CLI, `WMUX_CLI_METHODS`) and `wmux-hook-bridge` (the agent lifecycle bridges under `integrations/**`, granted exactly `hooks.signal` — see `src/main/mcp/hookBridge.ts`). Both names are in `NON_IDENTIFYING_CLIENT_NAMES`: anyone may send them, which is precisely why each grants a bounded method set rather than a blanket bypass. The trusted in-process surfaces — the renderer `operator` bridge and the plugin host — are unaffected: they never carried an envelope and are exempt by positive in-process provenance, not by the grandfather.
