...
Requests without `clientName` are recorded as `legacy` — the recorder stays on as the evidence trail — but **the grandfather lane closes in the first release on or after **2026-09-30** (#1111)**. From that release an envelope-less request is refused by the permission gate with `rejection.reason = 'identity-status'`, `status = 'legacy'`; there is no approval path back in, because an anonymous caller has no identity to approve. `wmux-cli` remains as the honestly-labelled limited lane (curated allowlist, no approval dialog). The trusted in-process surfaces — the renderer `operator` bridge and the plugin host — are unaffected: they never carried an envelope and are exempt by positive in-process provenance, not by the grandfather.
