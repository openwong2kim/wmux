...
Requests without `clientName` **on the external wire** are recorded as `legacy` — the trusted in-process surfaces (renderer `operator` bridge, iframe plugin host) send no `clientName` by design and are not recorded. The recorder stays on as the evidence trail, but **the grandfather lane closes in the first release on or after 2026-09-30** (#1111). From that release an envelope-less wire request is refused by the permission gate with `rejection.reason = 'identity-status'`, `status = 'legacy'`; there is no approval path back in, because an anonymous caller has no identity to approve. `wmux-cli` remains as the honestly-labelled limited lane (curated allowlist, no approval dialog). The in-process surfaces are unaffected by the close: they never carried an envelope and are exempt by positive in-process provenance, not by the grandfather.
