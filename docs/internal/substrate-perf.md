...
- **`clientName` = `wmux-cli`.** The bench used to send none: the request was recorded as `legacy` by `RpcRouter` and grandfathered, so it ran against the production **enforce-mode** app without tripping an approval dialog. #1111 closes that lane in the first release on or after 2026-09-30, so the bench now sends `wmux-cli` and rides the curated internal-CLI lane (`src/main/mcp/internalCli.ts`) — a fixed method set, still no approval dialog. **Known gap:** `events.poll`, `pane.getMetadata`, `pane.setMetadata` (substrate-bench) and `pane.close`, `mcp.claimWorkspace` (perf-bench) are not in `WMUX_CLI_METHODS`; those scenarios need either an allowlist extension or the identity+approval flow before the close date.
