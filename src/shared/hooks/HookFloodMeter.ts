...
export class HookFloodMeter {
  ...
  /**
   * Snapshot the current window WITHOUT resetting. For read-only consumers
   * (the daemon's `daemon.hooks.health` RPC) that must not steal the window
   * out from under the periodic logger — `flush` is destructive, so a poller
   * calling it would blank the next log line every time it ran.
   *
   * The returned window is therefore PARTIAL: it covers only the time since
   * the last flush, not a full `windowMs`.
   */
  peek(windowMs: number): HookFloodSummary | null {
  ...
  flush(windowMs: number): HookFloodSummary | null {
    const summary = this.peek(windowMs);
