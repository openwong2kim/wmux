...
export interface ParsedTerminalPrompt {
  ...
  commandLines: string[];
  /** Every command row, joined, untruncated. What a summary must carry whole. */
  commandText: string;
...
/** Plain `Yes` / `No`, nothing else: case-insensitive, trimmed, exact. */
 ...
 *
 * Only the plain Yes and plain No options are ever answerable. Anything else —
 * "Yes, and don't ask again for … commands", which writes a PERMANENT allow
 * rule — stays display-only. No plain Yes, a dialog taller than the viewport,
...
export function terminalPromptAnswerability(
  ...
  const choices = parsed.options
    .filter((o) => PLAIN_YES.test(o.label.trim()) || PLAIN_NO.test(o.label.trim()))
    .map((o) => ({ key: o.key, label: o.label.trim() }));
  const hasYes = choices.some((c) => PLAIN_YES.test(c.label));
