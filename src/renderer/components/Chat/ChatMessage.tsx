...
function Body({ eventId, body, label }: { eventId: string; body: ToolBody | CodeBlockRef; label: string }) {
  ...
  const [loaded, setLoaded] = useState(initial !== undefined && !body.truncated);
...
export function ChatMessage() {
  ...
  if (event.kind === 'meta') return <MessagePrimitive.Root className="wmux-chat-meta">{event.label}</MessagePrimitive.Root>;
  if (event.kind === 'tool_use' || event.kind === 'tool_result') {
    const output = event.kind === 'tool_result' ? event : result;
    return <MessagePrimitive.Root className="wmux-chat-tool">
  ...
  return <MessagePrimitive.Root className={`wmux-chat-message ${user ? 'wmux-chat-user' : 'wmux-chat-assistant'}`}>
    {user ? <div className="wmux-chat-user-text">{event.text}{event.hasImage && <p>{t('chat.imageInTerminal')}</p>}</div>
      ...
      : <div className="wmux-chat-prose"><Prose event={event} /></div>}
  </MessagePrimitive.Root>;
}
