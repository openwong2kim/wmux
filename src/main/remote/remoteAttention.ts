...
export function formatRemoteAttention(
  ...
  if (kind === 'critical') {
    return {
      ...
      body: str(data.action) || 'A pane is waiting on you.',
      type: 'warning',
      category: 'approval',
    };
  }
  if (kind === 'approval') {
    ...
    const summary = str(data.toolInputSummary);
...
export class RemoteAttentionGate {
  ...
  consume(event: string, data: string): RemoteAttentionNotification | null {
    if (event === 'reset') {
      ...
      const headId = parsed && typeof parsed.headId === 'number' ? parsed.headId : 0;
      this.replayUntilId = headId > 0 ? headId : 0;
      return null;
    }
