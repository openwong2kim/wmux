...
describe.skipIf(!onWindows)(
  "fan-out launch line in a real PowerShell (#1490)",
  ...
  () => {
    ...
    function launch(shell: string, prelude: string, prompt: string): string[] {
      ...
      execFileSync(
        ...
        [
          ...
          "-NonInteractive",
          "-EncodedCommand",
          Buffer.from(prelude + line, "utf16le").toString("base64"),
        ],
        {
    ...
    const expectedFlags = (
      workerLaunchFlags("auto").match(/"[^"]*"|\S+/g) ?? []
    ).map((w) => w.replace(/^"|"$/g, ""));
    ...
    for (const [shellName, shell, prelude] of shells) {
      for (const [promptName, prompt] of Object.entries(PROMPTS)) {
        it(`${shellName}: ${promptName} arrives as one exact argument`, () => {
          expect(launch(shell, prelude, prompt)).toEqual([
            prompt,
            ...expectedFlags,
          ]);
        });
