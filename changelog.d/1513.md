...
- On a fresh install, the hooks install prompt no longer opens on top of the Welcome dialog, which already offers the same install. If the hooks are still missing once the Welcome dialog closes, a pending ask is checked again first; otherwise the prompt returns on a later launch.
