...
- **`browser_snapshot` can see menus that only exist on hover.** A nav item
  ...
  item is now marked `has-submenu`, worked out from the page's own stylesheets
  and ARIA without touching the page, and a line at the end of the snapshot says
  how many were found. Pass `probeHover:true` and wmux also hovers the top few
  and lists what each one reveals — `[hover first: Docs | API | Pricing]` —
  costing up to ~2.5 s and moving the pointer, which is why it is opt-in. The
  pointer is put back afterwards and the menu is checked to have closed again;
  if one stayed open, the line says so. Nothing is ever marked on a guess: a
  form field, a disabled or inert control, and anything behind a modal overlay
  are all left alone, and so is a hover rule that merely nudges an icon that was
  already on screen. (#1415)
