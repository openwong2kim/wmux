...
- The UI now renders in Inter (400–600), the typeface the design system has
  always specified. It is bundled with the app under the SIL Open Font License
  (Latin + Latin Extended, +131 KB), so it looks the same on every machine
  instead of falling back to whatever the OS supplies. Text outside those
  scripts, including Hangul, still uses the system face.
...
- Modal dimming, inset highlights and hairlines now derive from theme tokens
  ...
  lit the wrong way, and 31 colours carried a hardcoded fallback from an
  unrelated palette that would have surfaced if a theme token ever went
  missing.
