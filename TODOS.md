...
## Web token hardening follow-ups (P3, from PR #634 3-model panel, 2026-07-26)
- **What:** Optional hardening surfaced by the review panel on the durable-web-stop fix (#634, #620). None block anything — all "nice-to-have" tightening, no cross-model consensus.
  1. Token write via `O_EXCL` fresh inode + `rename` (instead of placeholder→harden→populate in place) — closes the residual same-user sub-second exposure window entirely (Codex #1–3). #634 already improved on baseline here.
  2. `fsync` the file + parent dir on the revoke path (unlink / disabled-overwrite) before acknowledging — power-loss-during-write durability, a guarantee neither old tmp+rename nor new code provided (Codex #5).
  3. Serialize the web `start`/`stop` RPC lifecycle with a generation token / mutex — **pre-existing** concurrency gap, not introduced by #634 (Codex #4).
  4. WebToggle now renders `info.error` verbatim (intentional, to show the real failure) — decide whether to i18n-wrap it for locale consistency (no security/XSS issue; React escapes it).
- **Why deferred:** GLM's only would-be blocker (Windows `statSync.ino` breakage) was empirically refuted by #634's own green windows-latest CI running the exact new test. Remaining items are narrow same-user windows or pre-existing gaps; bundle 1+2 next time web-auth code is touched.
- **Context:** PR #634 merged aa634513. **Priority:** P3.
