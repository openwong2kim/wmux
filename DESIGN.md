...
| 2026-08-29 | Future prompt scheduling lives in the workspace-spanning agent toolbar as a quiet clock action, not in Command Deck schedules. Its popover remains available when the agent exits so queued work can still be paused or deleted; creating a new row requires a currently detected agent | The target and execution contract are per-session: one immutable PTY plus one verified agent family. Command Deck schedules start new workspace-orchestrator turns and carry different autonomy semantics. Sharing their surface would make blast radius ambiguous, while disabling the session control when an agent disappears would strand unavailable schedules |
