...
| 2026-07-28 | Chat View conversation layout: USER turns are right-aligned and width-capped (~78%) in a **raised card** (faint surface fill + 1px `color-mix(text-main 10%)` hairline + top 1px inset highlight, 7px card radius); agent turns stay left and full width. This **revises "Hierarchy from typography, not decoration" (Typography) for user-turn ALIGNMENT ONLY** — everywhere else in Chat View, including the speaker labels, hierarchy stays typographic | Owner decision from a live run: in a conversation the reader has to tell their own messages from the agent's at a glance, and side + a raised edge is the convention that does it. The treatment is the existing gpui *raised* surfacing, NOT a coloured chat bubble, so "amber never fills areas" and the no-wash rule are preserved intact; tool lines, code/diff chips and the trust seam stay left and full width because they are machine evidence, not conversation |
| 2026-07-27 | Chat View ships as an opt-in per-surface VIEW MODE (not a new pane tab, not a default). Terminal stays the default rendering and the ground truth; the trust seam links back on every chat surface | Terminal-first identity (2026-07-11) is unchanged — Chat is a supervision lens over the transcript the daemon already collects, not a chat-first pivot. Per-surface toggle (rather than a pane tab) keeps the 2026-07-20 pane-tab scope intact and is what makes the keep-warm memory policy coherent |
| 2026-07-28 | The Chat View toggle lives in the surface-tab CONTEXT MENU (right-click a pane's tab or its label), not as a `Terminal \| Chat` segment in the tab strip. `Cmd/Ctrl+Shift+J` and a command-palette entry stay as the discoverable, chrome-free paths. A pane whose agent publishes no transcript gets NO menu row (not a greyed one); the transient "after the first turn" state keeps a greyed row with its reason. Revises the 2026-07-27 row's placement only — Chat is still an opt-in per-surface view mode | Owner decision from a live run: the segment plus three icons plus zoom nearly filled the strip, so a workspace split four to six ways spent more of the pane on chrome than on content. Right-click is already this app's menu gesture (README: smart right-click), it reaches surfaces in UNFOCUSED panes that the chord cannot, and a menu row can be absent where a segment can only be greyed — so the Claude-only v1 limit stops paying for permanent dead space |
