...
| 2026-08-14 | 덱 헤더=아이콘 스트립(Agent·Git·Channels·web, 36px 글리프), 접힌 덱=덱 엣지의 36px 세로 글리프 레일, 사이드바 발치의 Agent·Git·Channels·web 라벨 행 제거 | 텍스트 탭 3개가 248–320px 컬럼의 헤더를 통째로 먹었다. 진입점은 반대편(사이드바 발치)에 있었고 사이드바를 접으면 아예 사라졌다(MiniSidebar에 없음) — 덱을 여는 것들은 덱의 엣지에 산다. 이름·현재 모델은 툴팁/접근가능 이름으로 이동 |
