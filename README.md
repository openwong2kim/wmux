...
| ⏰ **Prompts arrive when the quota resets** | Schedule an exact prompt for one detected agent session — including a one-click **+5h** target. wmux persists it, waits through running turns and approval prompts, re-verifies that the same agent still owns the PTY, then safely pastes and submits when the session is ready. |
...
**Per-session prompt schedules** — from a detected agent pane, queue an exact prompt for a local future time or use the +1h / +5h / +24h shortcuts; one-shot and repeating schedules persist across app restarts. Delivery is bound to the original PTY and detected agent family, waits while a turn or approval is active, and uses safe bracketed paste before submit. Existing schedules remain manageable even while their agent is unavailable.
